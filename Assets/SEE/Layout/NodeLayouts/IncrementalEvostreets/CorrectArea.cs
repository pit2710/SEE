...
namespace SEE.Layout.NodeLayouts.IncrementalEvostreets
{
    ...
    internal static class CorrectAreas
    {
        ...
        public static bool Correct(IList<Node> nodes, IncrementalEvostreetsAttributes settings)
        {
            if (nodes.Count == 1)
            {
                return true;
        ...
        /// <returns>true if correction was successful, else false</returns>
        private static bool GradientDecent(IList<Node> nodes, IncrementalEvostreetsAttributes settings)
        {
